...
def scrape_visa():
    ...
    payload = {"city":["Bangalore","Mumbai"],"from":0,"size":1,"sort":{"createdOn":"DESC"}}
    #response = requests.get()
    response = requests.post(url, headers=headers,json=payload)
    resp = response.json()
    hits = (resp.get("jobDetails"))
    jobs = 25
    all_jobs=[]
    for hit in hits:
        #print(hit)
        ...
        responsibilities ='responsibilities' #hit.get("jobDescription")
        qualifications = hit.get("qualifications")
        location=hit.get("city")
        timestamp =hit.get("createdOn")
        posting_date =datetime.strptime(timestamp , "%Y-%m-%dT%H:%M:%S.%f%z").date()
        created_date = hit.get("createdOn")
        JobFunction = 'JobFunction'
        JobFamily = hit.get("department")
        apply_url=hit.get("applyUrl")
        #print(posting_date) 
        #print(title)
        all_jobs.append({
            "company": "Visa",
            ...
            "update_date" : 'Null',
            "apply_link": 'https://jobs.smartrecruiters.com/Visa/' +  job_id                       
            
        })
        jobs = jobs-1
        if jobs <=0 :
            break
    print(len(all_jobs) , ' Visa jobs added')
    return all_jobs
